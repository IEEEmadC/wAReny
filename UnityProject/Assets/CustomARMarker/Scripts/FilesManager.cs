...
namespace Sample
{
    public class FilesManager : MonoBehaviour
    {
        ...
        public void StartTakePhoto()
        {
            ...
            ui.confirm.gameObject.SetActive(true);
            ui.delete.gameObject.SetActive(true);
            StartCoroutine(ImageASsign());
        ...
        IEnumerator ImageCreate()
        {
            ...
            photo.Apply();
            tmp=new Texture2D(Screen.width / 2, Screen.height / 2, TextureFormat.RGB24, false);
            tmp.ReadPixels(new Rect(Screen.width / 4, Screen.height / 4, Screen.width / 2, Screen.height / 2), 0, 0, false);
            tmp.Apply();
            ui.img.texture = (Texture)tmp;
